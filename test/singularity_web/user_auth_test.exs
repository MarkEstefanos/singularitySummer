defmodule SingularityWeb.UserAuthTest do
  use SingularityWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Singularity.Accounts
  alias Singularity.Accounts.Scope
  alias SingularityWeb.UserAuth

  import Singularity.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SingularityWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SingularityWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SingularityWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
